...
Date: 2024-10-14
Description: This script fine tunes the Bert Model using SQuAD dataset with p-tuning approach.
Version: 1.0
"""
import transformers
...
def main():
    print_gpu_info()
    # This flag indicates whether to use SQuAD v2 (if impossible answers are allowed)
    squad_v2 = False
    ...
    # Load dataset from the previously saved directory
    datasets = load_from_disk("./squad_data")
    # # Select 1000 samples from the train set
    # train_subset = datasets['train'].select(range(1000))

    # # Select 1000 samples from the validation set
    # validation_subset = datasets['validation'].select(range(1000))

    # # Verify the number of rows in the subsets
    ...
    # # Assuming train_subset and validation_subset are already created
    ...
    doc_stride = 128   # Authorized overlap between two parts of the context when splitting
    pad_on_right = tokenizer.padding_side == "right"

    # Tokenizing the datasets
    tokenized_datasets = datasets.map(lambda examples: prepare_train_features(
    ...
    with mlflow.start_run() as run:
        ...
        args = TrainingArguments(
            f"{model_name}-prompt-encoder-finetuned-squad",
        ...
        #peft_config = PromptEncoderConfig(task_type=TaskType.QUESTION_ANS, num_virtual_tokens=20, encoder_hidden_size=128)
        #peft_config = PrefixTuningConfig(task_type=TaskType.QUESTION_ANS, num_virtual_tokens=20)
        peft_config = PromptEncoderConfig(task_type=TaskType.QUESTION_ANS, num_virtual_tokens=20, encoder_hidden_size=128)

        model = get_peft_model(model, peft_config)


        # Enable gradient computation for the LoRA layers
        ...
        validation_features = datasets["validation"].map(
            lambda examples: prepare_validation_features(examples, tokenizer, pad_on_right, max_length, doc_stride),
            batched=True,
        ...
        final_predictions = postprocess_qa_predictions(datasets["validation"], tokenizer, validation_features, raw_predictions.predictions, squad_v2)
        ...
        # Log the model
        mlflow.pytorch.log_model(model, "prompt-encoder-fine-tuned-model")
